...
fn is_between(a: r32, b: r32, c: r32) -> bool {
    if b == c {
        a == b
    } else if b < c {
        b < a && a < c
        ...
        if !vertices_left.is_empty() || !vertices_right.is_empty() {
            ...
            for edge in &self.lines_for_edges() {
                if let Some(intsn) = intersection(*edge, line)
                    && is_between(intsn.x, edge.point1.x, edge.point2.x)
                    && is_between(intsn.y, edge.point1.y, edge.point2.y)
                {
...
pub fn advance(a: u32, b: u32, c: u32, pt: Point) -> Result<(Point, Letter), String> {
    let projected_cube = projected_cube(a, b, c).unwrap();
    // Consider: _ /
    //            |
    if !projected_cube.has_point_inside(pt) {
        Err(format!(
            "Point passed to `advance`, {}, is not inside the projected cube",
            pt
        ))
    } else {
            ...
            && projected_y_z_equals_1_1.point_line_config(pt) == PointLineConfiguration::Left
        {
            ...
            Err(format!(
                "Point passed to `advance`, {}, is inside the projected cube but falls on a projected constraint plane",
                pt
            ))
        }
    }
