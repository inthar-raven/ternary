        ...
        if multiplicity == 1 || scale.is_empty() || !scale.len().is_multiple_of(multiplicity) {
            ...
            if !coprime_part.is_multiple_of(multiplicity) {
                ...
                if chain_length == 1 {
                    ...
                    // We'll need the enumeration index later
                    let gener_chains_enumerated: Vec<(usize, Vec<CountVector<usize>>)> = (0..scale
                        .len())
                        .map(|degree| {
                            ...
                            (
                                ...
                                gener_chains_enumerated
                                    ...
                                    .filter(|(_, gener_chain)| {
                                        ...
                                        // println!(
                                        //     "weak_period_pattern: {:?}",
                                        //     weak_period_pattern(&gener_chain[..chain_length - 1])
                                        // );
